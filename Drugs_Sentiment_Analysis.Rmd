...
TotalDrugsDataEdited<-TotalDrugsDataEdited %>% mutate(sentimentlabel =
                     case_when(rating <= 4 ~ "negative", 
                               rating < 8 & rating>4 ~ "neutral",
                               rating <= 10 & rating > 7 ~ "positive")
)
...
head(TotalDrugsDataEditedCorpus)
...
classifier = naiveBayes(x = training_set[-2],y = training_set$sentimentlabel)
...
# head(training_set)
# ## Logistic Regression
# logistic_model <- glm(sentimentlabel~ condition+rating,
#                     data = training_set,
#                     family = "binomial")
# 
# 
# predict_reg <- predict(logistic_model,
#                        test_set[-1], type = "response")
